...
import io.aiven.kafka.connect.opensearch.BehaviorOnLargeMessage;
        ...
        if (isNull(record.value())) {
            ...
            if (payload.length > maxBatchPayloadBytes) {
                switch (config.behaviorOnLargeMessage()) {
                    case FAIL :
                        LOGGER.error(
                                "Document size of {} bytes exceeds max.batch.payload.bytes of {} bytes. Stopping.",
                    ...
                    case SKIP :
                        LOGGER.debug(
                                "Document {}/{} size {} bytes exceeds max.batch.payload.bytes {} bytes. Skipping.",
                                record.kafkaPartition(), record.kafkaOffset(), payload.length, maxBatchPayloadBytes);
                        return null;
                    case PASS :
                    default :
                        LOGGER.warn(
                                "Document size of {} bytes exceeds max.batch.payload.bytes of {} bytes. Passing.",
                                payload.length, maxBatchPayloadBytes);
                        break;
                }
            }
